...
v7 — RISEFALL LSTM ENSEMBLE (Gate 6):
    ...
    Optional env vars (all have working defaults):
        ...
        LSTM_MIN_EDGE_FOR_MINUTE   default 0.08 -- min edge to trade a
                                    minute contract instead of the default
                                    tick contract
...
# confident, low-uncertainty edge AND agrees with the layer stack's
# direction, this bot will actually attempt a minute-duration Rise/Fall
# contract instead of the default tick contract for that trade -- see
# lstm_evaluate() and its Gate 6 call sites. If Deriv rejects duration_
# unit="m" for that particular symbol (some contracts only offer tick
# durations), execute_single_step() transparently falls back to the
# tick-based MC pick in the same cycle rather than dropping the trade --
# no separate contracts_for probing needed, the buy attempt is the probe.
...
# Minimum |p-0.5| edge, at the minute model's own best duration, required
# before we'll actually swap the trade over to a minute contract. Set well
...
def hmm_gbm_terminal_log_returns(hmm_model, recent_returns: np.ndarray,
    ...
    sd_path = np.clip(sds[state_idx] * math.sqrt(n_steps), 1e-9, None)
    ...
    for dur in candidate_durations:
        ...
        if len(returns) >= BOOTSTRAP_BLOCK_SIZE * 3:
            ...
            for i in range(n_boot):
                ...
                sampled = np.concatenate([returns[j:j + BOOTSTRAP_BLOCK_SIZE] for j in idx])[:dur]
...
async def execute_single_step(client, state, symbol, direction, stake, step, duration=5,
                              duration_unit="t", fallback_duration=None,
                              fallback_duration_unit="t", feats=None):
    ...
    duration_unit: normally "t" (ticks). v7's Gate 6 can pass "m" (minutes)
    here when the RISEFALL LSTM minute-bar model confidently prefers a
    minute-duration contract for this trade (see lstm_evaluate()). Not
    every symbol/contract necessarily supports duration_unit="m" via the
    Options API buy call, and there's no cheap way to know that in advance
    without a live probe — so rather than pre-classifying eligibility, we
    just attempt the buy and, if Deriv rejects it, fall back IN THE SAME
    CYCLE to `fallback_duration`/`fallback_duration_unit` (the tick-based
    Monte Carlo pick that already cleared every other gate) instead of
    ...
    try:
        try:
            ...
            if duration_unit != "t" and fallback_duration is not None:
                print(f"[Trade] {symbol} duration_unit={duration_unit!r} rejected "
                      f"by Deriv ({e}) — falling back to {fallback_duration}"
                      f"{fallback_duration_unit} tick contract this cycle.")
                contract_id = await buy_contract(
                    client, symbol, direction, int(fallback_duration),
                    fallback_duration_unit, stake)
            else:
                raise
