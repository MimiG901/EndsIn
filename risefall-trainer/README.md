...
RISEFALL **minute** model every 2 hours and uploads it to Supabase, for the
...
container every 2 hours (UTC), runs `python risefall_lstm_train.py`
directly (`MODEL_KIND=minute` from the environment), and the container
exits when done. Railway does not start the next scheduled run until the
...
2. **Minute mode needs far more raw ticks per symbol than tick mode.**
   ...
   anchor striding is applied. `LSTM_MAX_TICKS` now defaults to 1,800,000
...
- `LSTM_MAX_TICKS` defaults to 1,800,000 total for minute mode (vs
  300,000 for tick mode) — minute mode needs far more raw ticks per
  symbol to accumulate enough distinct one-minute bars. This is a bigger
  `ticks_history` pull than the old tick-mode default; expect the fetch
  stage alone to take several minutes across a 10-symbol basket.
- Each run trains 3 torch models (the served ensemble + the GRU/CNN
  ...
  keeps it well within a 2h cron window at the defaults above.
